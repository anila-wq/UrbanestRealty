  ...
  const renderMobileProjectCard = (project: any, isClickable: boolean = false, containerRef?: React.RefObject<HTMLDivElement>, isDesktop: boolean = false) => {
    const cardContent = (
      <>
        ...
        <div 
          className={`relative overflow-hidden group ${isClickable && project.url ? 'cursor-pointer' : ''}`}
          onClick={isClickable && project.url ? () => handleProjectClick(project.name, project.url) : undefined}
        >
          <div className={isClickable && project.url ? 'pointer-events-none' : ''}>
            <ImageWithFallback
              src={project.image}
              alt={project.name}
          ...
          {/* Clickable scroll navigation arrows - show for all projects with containerRef */}
          {containerRef && (
            <>
              <button 
                onClick={(e) => {
                ...
                className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 rounded-full p-2.5 transition-all z-10 shadow-xl pointer-events-auto"
              ...
              <button 
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 rounded-full p-2.5 transition-all z-10 shadow-xl pointer-events-auto"
        ...
        <CardContent className="pt-3 pb-4 text-center">
          ...
          <div className="text-gray-900 font-bold text-base mb-2 text-center min-h-[24px]">
            {project.status !== 'Upcoming' ? project.price : '\u00A0'}
          </div>
        ...
        <div className="hidden md:block">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="border border-gray-200 rounded-lg p-6 bg-gray-50">
              ...
              <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scrollbar-hide overflow-hidden" ref={ongoingScrollRefDesktop} style={{ maxWidth: '300px', margin: '0 auto' }}>
                {ongoingProjects.map((project) => (
                  <div key={project.id} className="flex-shrink-0 w-[300px] snap-start">
            ...
            <div className="border border-gray-200 rounded-lg p-6 bg-gray-50">
              ...
              <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scrollbar-hide overflow-hidden" ref={upcomingScrollRefDesktop} style={{ maxWidth: '300px', margin: '0 auto' }}>
                {upcomingProjects.map((project) => (
                  <div key={project.id} className="flex-shrink-0 w-[300px] snap-start">
            ...
            <div className="border border-gray-200 rounded-lg p-6 bg-gray-50">
              ...
              <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scrollbar-hide overflow-hidden" ref={completedScrollRefDesktop} style={{ maxWidth: '300px', margin: '0 auto' }}>
                {completedProjects.map((project) => (
                  <div key={project.id} className="flex-shrink-0 w-[300px] snap-start">
